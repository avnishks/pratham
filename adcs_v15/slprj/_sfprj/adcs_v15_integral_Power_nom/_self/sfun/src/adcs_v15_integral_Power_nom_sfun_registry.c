...
unsigned int sf_process_check_sum_call( int nlhs, mxArray * plhs[], int nrhs,
  const mxArray * prhs[] )
{
  extern unsigned int sf_adcs_v15_integral_Power_nom_process_check_sum_call( int
    nlhs, mxArray * plhs[], int nrhs, const mxArray * prhs[] );
  if (sf_adcs_v15_integral_Power_nom_process_check_sum_call(nlhs,plhs,nrhs,prhs))
    return 1;
  return 0;
}
...
{
  extern unsigned int sf_adcs_v15_integral_Power_nom_autoinheritance_info( int
    nlhs, mxArray * plhs[], int nrhs, const mxArray * prhs[] );
  if (sf_adcs_v15_integral_Power_nom_autoinheritance_info(nlhs,plhs,nrhs,prhs))
    return 1;
  return 0;
}
...
{
  extern unsigned int
    sf_adcs_v15_integral_Power_nom_get_eml_resolved_functions_info( int nlhs,
  ...
  if (strcmp(machineName, "adcs_v15_integral_Power_nom") == 0) {
    const mxArray *newRhs[2] = { NULL, NULL };

    newRhs[0] = prhs[0];
    newRhs[1] = prhs[2];
    return sf_adcs_v15_integral_Power_nom_get_eml_resolved_functions_info(nlhs,
      plhs,2,newRhs);
  }
...
extern unsigned int sf_adcs_v15_integral_Power_nom_method_dispatcher(SimStruct
...
{
  if (!strcmp(machineName,"adcs_v15_integral_Power_nom")) {
    return(sf_adcs_v15_integral_Power_nom_method_dispatcher(simstructPtr,
...
extern void adcs_v15_integral_Power_nom_terminator(void);
void sf_machine_global_terminator(void)
{
  if (sfMachineGlobalTerminatorCallable) {
    ...
    adcs_v15_integral_Power_nom_terminator();
    sf_debug_terminate();
  }
...
extern void adcs_v15_integral_Power_nom_initializer(void);
extern void adcs_v15_integral_Power_nom_register_exported_symbols(SimStruct* S);
extern void adcs_v15_integral_Power_nom_debug_initialize(void);
void sf_register_machine_exported_symbols(SimStruct* S)
{
  adcs_v15_integral_Power_nom_register_exported_symbols(S);
}
...
void sf_machine_global_initializer(SimStruct* S)
{
  ...
  if (sfMachineGlobalInitializerCallable) {
    ...
    if (!simModeIsRTWGen) {
      adcs_v15_integral_Power_nom_debug_initialize();
    }

    adcs_v15_integral_Power_nom_initializer();
  }
